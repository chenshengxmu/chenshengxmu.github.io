---
layout: post
title: Check if a string has all characters with same frequency with one variation allowed
---
...
Output : Yes
We can make it valid by removing "a"
...
* The idea is to use a frequency array that stores frequencies of all characters. Once we have frequencies of all characters in an array, we check if count of total different and non zero values are not more than 2. Also, one of the counts of two allowed different frequencies must be less than or equal to 2. Below is C++ implementation of idea.
{% highlight cpp %}
